...
# Gráficos iniciales
fig_sentimientos = px.pie(
    data, names="sentiment", title="Distribución de Sentimientos"
)
fig_tiempo = px.histogram(
    data, x="created_at", color="sentiment",
    title="Sentimientos a lo largo del tiempo", nbins=30
)
...
def update_graph(start_date):
    ...
    # Obtener el valor máximo de la fecha para el end_date
    end_date = filtered_data['created_at'].max().date()

    # Actualizar el gráfico de tiempo
    fig_tiempo = px.histogram(
        filtered_data, x="created_at", color="sentiment",
        title="Sentimientos a lo largo del tiempo", nbins=30
    )
